...
if __name__ == "__main__":
    ...
    df_train = df_needed[df_needed['TARGET'].notna()]
    ...
    # Le Model, prédiction et API Build
...
def train_xgb_model(train_data):
    ...
    # Normalize resampled data with Min-Max Scaler
    scaler = MinMaxScaler()
    X_train_resampled_normalized = scaler.fit_transform(X_train_resampled)

    # Train the XGBoost model with the normalized data
    model = XGBClassifier()
    model.fit(X_train_resampled_normalized, y_train_resampled)

    return model, scaler

# Function to make predictions
def make_predictions(model, scaler, test_data, threshold=0.35):
    X_test = test_data.drop(columns=['TARGET'])

    # Normalize test data with the same Min-Max Scaler used for training
    X_test_normalized = scaler.transform(X_test)

    # Get prediction probabilities
    probabilities = model.predict_proba(X_test_normalized)[:, 1]

    # Apply the optimal probability threshold (0.35)
...
def predict_endpoint():
    ...
    # Load the datasets
    train_data = pd.DataFrame(input_data['train_data'])
    test_data = pd.DataFrame(input_data['test_data'])

    # Train the XGBoost model
    model, scaler = train_xgb_model(train_data)
...
# Additional route to handle external prediction requests
@app.route('/external_predict', methods=['POST'])
def external_predict_endpoint():
    # Extract input data from the request
    input_data = request.get_json()

    # Make an external POST request to the Flask API for predictions
    url = 'http://127.0.0.1:5000/predict'  # Update with the correct URL
